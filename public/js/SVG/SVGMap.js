var SVGMap = /** @class */ (function () {
    ...
    SVGMap.prototype.calculateAutoGroups = function () {
        if (this.zoomlevel >= this.MAX_GROUP_LEVEL)
            return;
        this.drawGuides();
        var late_removal = Array.apply(null, Array(20)).map(function (element) { return []; });
        for (var level = 0; level < this.data.groups.length; level++) {
            for (var i = 0; i < this.data.groups[level].length; i++) {
                var group = this.data.groups[level][i];
                if (group.auto) {
                    late_removal[level].push(group.id);
                }
            }
        }
        for (var i = 0; i < late_removal.length; i++) {
            var level = late_removal[i];
            for (var _i = 0, level_1 = level; _i < level_1.length; _i++) {
                var id = level_1[_i];
                $("#gmarker-" + id).remove();
                var index = -1;
                for (var j = 0; index != -1 && i < this.data.groups[i].length; j++) {
        ...
        for (var _a = 0, _b = this.svg.select('.jails rect').members; _a < _b.length; _a++) {
            var jail = _b[_a];
    ...
    SVGMap.prototype.groupMarkers = function (level) {
        ...
        for (var _i = 0, _a = this.marker_groups; _i < _a.length; _i++) {
            ...
            var _loop_2 = function (gmarker) {
                if (i == level) {
                    var fit = (gmarker.affects.toString().length == 1) ? 1 : gmarker.affects.toString().length / 2;
                    var a = this_1.svg.select('#SVG_MAIN_CONTENT').members[0].link('#gmarker-' + gmarker.id).attr('class', 'non-link gmarker').attr('id', 'gmarker-' + gmarker.id);
                    ;
                    ...
                    $(a.node).on('click', function (e) {
                        ...
                        self_1.zoomAndMove(x, y, self_1.zoomlevel + 2);
                    });
                    $(a.node).on('focus', function () {
                        $(this).on('keyup', function (e) {
                            if (e.which == 13) {
                                ...
                                self_1.zoomlevel += 2;
                                var _a = $(this).attr('data-coords').split(':'), x = _a[0], y = _a[1];
                                self_1.zoomAndMove(x, y, self_1.zoomlevel);
                            }
                        });
                    });
                }
                else {
                    for (var _i = 0, _a = this_1.svg.select('#gmarker-' + gmarker.id).members; _i < _a.length; _i++) {
                        var member = _a[_i];
                        member.remove();
                    }
                }
        ...
        for (var _e = 0, late_removal_1 = late_removal; _e < late_removal_1.length; _e++) {
            var marker = late_removal_1[_e];
    ...
    */
    SVGMap.prototype.move = function (x, y, raisedbyuser) {
        ...
        setTimeout(function () {
            _this.groupMarkers(_this.zoomlevel);
            _this.updateSidebar();
        }, 400);
    };
    /*
